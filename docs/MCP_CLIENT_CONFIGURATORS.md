...
- **Typical JSON-file clients** (Cursor, VSCode GitHub Copilot, VSCode Insiders, Windsurf, Kiro, Trae, etc.).
...
- **Kiro**, **Trae** – JSON configs with project-specific paths and flags.
...
- **URL property name**
  ...
  - Example: Windsurf uses `"serverUrl"`.
- **VS Code layout**
...
Look at `CursorConfigurator`, `VSCodeConfigurator`, `VSCodeInsidersConfigurator`, `KiroConfigurator`, or `TraeConfigurator` for phrasing.
