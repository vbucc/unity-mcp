...
- **[`manage_build`](./manage_build.md)** — Manage Unity player builds — trigger builds, switch platforms, configure settings, manage build scenes and profiles, run batch builds across platforms. settings accepts any PlayerSettings property (e.g. bakeCollisionMeshes, allowUnsafeCo…
...
- **[`manage_profiler`](./manage_profiler.md)** — Unity Profiler: CPU hotspots, memory, counters, frame time, captures.
- **[`manage_project_settings`](./manage_project_settings.md)** — Read, write, and discover Unity project settings across categories: quality (QualitySettings), physics, physics2d, time, editor (EditorSettings).
